...
#if WINDOWS
            if (txtInput.Handler.PlatformView is Microsoft.UI.Xaml.Controls.TextBox textBox)
            {
...
#if ANDROID
            if(txtInput.Handler.PlatformView is AndroidX.AppCompat.Widget.AppCompatEditText editText)
            {
                using (var gradientDrawable = new Android.Graphics.Drawables.GradientDrawable())
                {
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
        ...
        {
            txtInput.Completed += InputCompleted;
            txtInput.Focused += InputFocused;
            txtInput.Unfocused += InputUnfocused;
        }
        ...
        protected void InputCompleted(object sender, EventArgs args)
        {
            ExecuteCommand();
            Completed?.Invoke(this, new EventArgs()); FocusNext();
        }

        protected void InputFocused(object sender, FocusEventArgs args)
        {
            var arg = new FocusEventArgs(this, true);
            FocusedCommand?.Execute(arg);
            Focused?.Invoke(this, arg);
        }

        protected void InputUnfocused(object sender, FocusEventArgs args)
        {
            var arg = new FocusEventArgs(this, false);
            UnfocusedCommand?.Execute(arg);
            Unfocused?.Invoke(this, arg);
        }
        ...
        private protected virtual Entry GetInputEntry()
        {
            return new Entry
            {
                ...
                PlaceholderColor = Colors.Gray.WithAlpha(.5f),
